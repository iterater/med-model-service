...
class ScoreModel(ChPatModel):
    ...
    def apply(self, patient_dict):
        ...
        riskS = self.calcSCOREindex(int(feature_vector[0]), feature_vector[1], feature_vector[2], float(feature_vector[3]), int(feature_vector[4]))
...
# =============================================================================
    ...
    def check_applicability(self, patient_dict):
        return ('age' in patient_dict) and ('sex' in patient_dict) and ('max_sbp' in patient_dict) and ('smoking' in patient_dict) and ('ah' in patient_dict) and ('ah_stage' in patient_dict) and ('height' in patient_dict) and ('weight' in patient_dict) 
        #return all(map(lambda col: col in patient_dict and patient_dict[col] is not None,
                      # self.cols_for_model))
    ...
    def apply(self, patient_dict):
        ...
        if riskTotal == 0:
            res_dict[self.result_name][-1]['value'] = "Риск отсутствует"
        if riskTotal == 1:
            res_dict[self.result_name][-1]['value'] = "Низкий риск"
            
        if riskTotal == 2:
            res_dict[self.result_name][-1]['value'] = "Умеренный риск"
            
        if riskTotal == 3:
            res_dict[self.result_name][-1]['value'] = "Высокий риск"
            
        if riskTotal == 4:
            res_dict[self.result_name][-1]['value'] = "Очень высокий риск"
    ...
    def getRiskFactorsCnt(self, params):
        ...
        if params.get("sex")=="male":
            ...
            if "hdl_cholesterol" in params.keys() and float(params.get("hdl_cholesterol"))<1.2:
                dislypid+=1
        if "smoking" in params.keys() and eval(params.get("smoking")):
            rfCnt+=1
            print("smoking")
        ...
        if "fbs" in params.keys() and float(params.get("fbs"))>=5.6 and float(params.get("fbs"))<=6.9:
            rfCnt+=1
            print("fbs")
        if "gtt" in params.keys() and eval(params.get("gtt")):
            rfCnt+=1
            print("gtt")
        if IMT>=30:
            rfCnt+=1
...
#%%
    def getPOMCnt(self, params):
        ...
        if "creatinine" in params.keys():
            ...
            if ("chronic_kidney_disease" in params.keys() and int(params.get("chronic_kidney_disease"))>2 and mdrd >= 30 and mdrd <= 60) or (kok_gau< 60) or (ckd_epi>= 30 and ckd_epi<=60):
                pomCnt+=1
                print("formulas")
        if "pulse_pressure" in params.keys() and int(params.get("pulse_pressure"))>=60:  ####!!!!!!!!!!!!!!!!!!!!! TODO: учесть пожтлой и старческий возраст
            pomCnt+=1
        ...
        if "carotid_wall_thickening" in params.keys() and eval(params.get("carotid_wall_thickening")):
            pomCnt+=1
            print("carotid_wall_thickening")
        if "microalbuminuria" in params.keys() and eval(params.get("microalbuminuria")):
            pomCnt+=1
            print("microalbuminuria")
        if "severe_retinopathy" in params.keys() and eval(params.get("severe_retinopathy")):
            pomCnt+=1
    ...
    def commonRiskSSZ(self, params):
        ...
        if AH > 0:
            ...
            if "cerebrovascular_disease" in params.keys():
                cvb = eval(params.get("cerebrovascular_disease"))
            ssz = False
            if "heart_failure" in params.keys() and int(params.get("heart_failure"))>1:
                ssz = True
            if "coronary_heart_disease" in params.keys() and eval(params.get("coronary_heart_disease")):
                ssz = True
            hbp = 0
            if "chronic_kidney_disease" in params.keys() :
                hbp = eval(params.get("chronic_kidney_disease"))
                
            diabetes = False
            if "diabetes" in params.keys():
                diabetes = eval(params.get("diabetes"))
